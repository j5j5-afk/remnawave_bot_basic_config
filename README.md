...
This repo contains basic configuration and commands to set up basic remnawave panel, subscription panel, nginx RELEASE_URL=$(curl -s https://api.github.com/repos/j5j5-afk/remnawave_bot_basic_config/releases/latest | grep tarball_url | cut -d'"' -f4+ 2 additional hosts related to a telegram bot that manages those
